...
* @Author:Dijiang
* @Description:
* @Date: Created in 21:26 2022/7/31
* @Modified By: Dijiang
 */
...
func NewBinaryProtocolWriter(w io.Writer, strict bool) *binaryProtocolWriter {
...
func NewBinaryProtocolReader(r io.Reader, strict bool) *binaryProtocolReader {
...
func getBytes(s string) *bytes.Buffer {
	// deal with space
	strings.ReplaceAll(s, " ", "")
	arr, err := hex.DecodeString(s)
	if err != nil {
		logrus.Warn("error decode string to bytes")
		return nil
	}
...
func main() {
	...
	buffer := getBytes(input)
	NewBinaryProtocolReader(buffer, false)
}
